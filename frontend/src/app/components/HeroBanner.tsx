...
const HeroBanner = () => {
    ...
    const [suggestionAddMore, setSuggestionAddMore] = useState(false)
    ...
    console.log(selectedLocation, 23423);
    ...
    return (
        <div className="w-full lg:h-[700px] h-[500px] relative ">
            {/* Enter */}
            ...
            <div className="w-full h-full flex flex-col justify-center items-center absolute lg:top-10 top-24 z-99">
                <div className="lg:w-[70%] w-[90%] lg:h-[70px] h-[170px] bg-white rounded-2xl flex flex-col lg:flex-row lg:justify-start justify-center lg:pt-0 pt-5 items-center px-3 relative rounded-tl-none gap-0 lg:gap-1.5">
                    ...
                    <div className="lg:w-[60%] w-full h-[60%] rounded-md bg-slate-100 outline-none border-none px-2 flex lg:justify-start justify-center items-center relative">
                        ...
                        {selectedLocation.length > 1 ? <button onClick={handleAddMore} className="w-8 h-8 rounded-full bg-secondary/45 text-sm font-semibold text-slate-950 flex justify-center items-center cursor-pointer hover:bg-secondary/65 duration-200">+{selectedLocation.length - 1}</button> : null}
                        {selectedLocation.length <= 1 || suggestionAddMore ? <input type="text" value={input} onChange={handleSuggestLocation} className="w-full outline-none border-none px-2 " placeholder="Enter Location to Search..." /> : <button className="p-1 text-sm text-slate-950 cursor-pointer font-semibold bg-primary/80 mx-1 hover:bg-primary/90 duration-200 " onClick={handleAddMore}>Add More +</button>}
                        {suggestion && <div className="absolute w-[70%] bg-white/90 z-99 top-10 left-0">
                            <ul className="w-full h-[250px] custom-scrollbar overflow-y-auto px-5 py-1">
                        ...
                        {suggestionAddMore &&
                            <div className="absolute w-[80%] bg-white/90 z-99 top-12 left-0">
                                <div className="w-full h-[250px] px-4 border-t-2 border-slate-400 mt-2">
                                    <div className="w-full h-[100px] overflow-y-auto custom-scrollbar flex justify-start items-center] flex-wrap">
                                    ...
                                    <div className="w-full h-[70px]">
                                        ...
                                        <ul className="w-full flex justify-start items-center gap-2">
                                            <li className="w-[100px] p-2 bg-slate-100 border-2 border-slate-300 rounded-md">Bahrain</li>
                                            <li className="w-[100px] p-2 bg-slate-100 border-2 border-slate-300 rounded-md">Adliya</li>
                                            <li className="w-[100px] p-2 bg-slate-100 border-2 border-slate-300 rounded-md">Barbar</li>
                                            <li className="w-[100px] p-2 bg-slate-100 border-2 border-slate-300 rounded-md">Janabia</li>
                                        </ul>
                                    </div>
                                    <div className="w-full h-[70px]">
